...
ANSWER: It has been an exciting project to work on. I struggled with some basic things like indexing a pandas dataframe. I spent about 10 hours working on it over 3 days.
...
The flags are:
-x: specifies the x values to be plotted. Default: column 1.
-y: specified the y values to be plotted. Default: column 2.
-strat : a column by which the plot can be stratified, identified by different colors. If not specified, the plot will contain a single series. Default: None
...
ANSWER: The input file is the famous "Iris" which has 5 tab separated columns. The first line contains column headers. The columns are: sepal width (cm), sepal length (cm), petal width (cm), petal length (cm), and class.
...
ANSWER: Here is an example command to create a stratified plot using iris.tsv. The OUTPUT file flag is "-out" and the file will be saved as "scatter_iris.png" in the current working directory.
...
ANSWER: The output file has sepal length (y-axis) plotted against sepal width (x-axis). The plot is stratified based on the class of the iris. The file is saved as scatter_iris.png
